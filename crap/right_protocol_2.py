...
class CRAP(StackingProtocol):
    ...
    def connection_made(self, transport):
        ...
        if self.mode == "client":
            self.client_privak= ec.generate_private_key(ec.SECP384R1(), default_backend())
            self.client_pubk = self.client_privak.public_key()
            ...
            self.serialized_NonceA = str(self.NonceA).encode('ASCII')
            ...
            data = self.client_pubk.public_bytes(Encoding.PEM, PublicFormat.SubjectPublicKeyInfo)
            ...
            secure_new_packet = HandshakePacket(status=0,pk = data,signature = sigA,nonce = self.NonceA,cert = client_cert,certChain=[team5_CA_cert])
            ...
            def process_craphandshake_packet(pkt):
                ...
                if self.mode == "server":
                    ...
                    if pkt.status == 0:
                        ...
                        self.server_privak = ec.generate_private_key(ec.SECP384R1(), default_backend())
                        self.server_pubk = self.server_privak.public_key()
                        ...
                        self.serialized_NonceB = str(self.NonceB).encode('ASCII')
                        self.serialized_NonceA = str(pkt.nonce).encode('ASCII')

                        #generate shared_key
                        recv_pbk = load_pem_public_key(pkt.pk, backend=default_backend())
                        self.shared_key = self.server_privak.exchange(ec.ECDH(), recv_pbk)

                        print("111111111111111111")
                        data = self.server_pubk.public_bytes(Encoding.PEM, PublicFormat.SubjectPublicKeyInfo)
                        ...
                        secure_new_packet = HandshakePacket(status=1,pk = data, signature = sigB,nonce = self.NonceB)
                        ...
                        cnonceSignature = self.server_sign_pvk.sign(self.serialized_NonceA, padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA256())
                    ...
                    if pkt.status == 1:
                        ...
                        snonceSignature = self.client_sign_pvk.sign(self.serialized_NonceB,padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH),hashes.SHA256())
                        ...
                        self.shared_key = self.client_privak.exchange(ec.ECDH(), recv_pubk)
