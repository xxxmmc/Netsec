...
class EscapeRoomObject:
    ...
    def __repr__(self):
        return self.name
def my_function(strnew):
    s.send(strnew.encode())
    print(strnew.encode())
class EscapeRoomCommandHandler:
    def __init__(self, room, player, output=my_function):
        self.room = room
        self.player = player
        self.output = output
...
class EscapeRoomGame:
    def __init__(self, command_handler_class=EscapeRoomCommandHandler, output=my_function):
    ...
    def command(self, command_string):
        if self.status == "void":
            ...
            if not self.player["alive"]:
                ...
                self.status = "escaped"
serverName = "192.168.200.52"
serverPort = 19002
s = socket.socket();
def main(args):
    s.connect((serverName,serverPort))
    s.send(b'Yu Mao From Team 5')
    print(s.recv(1024))
    s.send(b'look mirror')
    print(s.recv(1024))
    s.send(b'get hairpin')
    print(s.recv(1024))
    s.send(b'unlock door with hairpin')
    print(s.recv(1024))
    s.send(b'open door')
    time.sleep(0.25)
  #  s.accept()
