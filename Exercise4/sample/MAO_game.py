...
class EscapeRoomCommandHandler:
    ...
    def _cmd_hit(self,hit_args):
        ...
        hit_success = False
        if len(hit_args) == 0:
            hit_result = "Hit yourself?Are you kidding?"
        elif len(hit_args) == 1:
            ...
            if not object or not object["visible"]:
                ...
                hit_machine = self.player["container"].get(hit_args[-1], None)
                if not hit_machine:
                    hit_result = "You don't have a {}".format(unlock_args[-1])
                else:
                    hit_success = True
        if hit_sucess:
            hit_result = "You have captured the flying key"
            object["visible"] = False
            self._run_triggers(object,"hit",hit_machine)
...
class EscapeRoomGame:
    ...
    def create_game(self, cheat=False):
        ...
        flyingkey = EscapeRoomObject("flyingkey",visible=True,gettable=False)
        door  =  EscapeRoomObject("door",   visible=True, openable=True, open=False, keyed=True, locked=True, unlockers=[hairpin,key])
        ...
        room["container"]  = create_container_contents(player, door,key,flyingkey, clock, mirror, hairpin, chest)
    ...
    def start(self):
        self.status = "playing"
        self.output("Where are you? You don't know how you got here... Were you kidnapped? There is a key lying there.Better take a look around")
