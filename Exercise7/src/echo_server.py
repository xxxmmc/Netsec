...
class EchoServer(asyncio.Protocol):
        ...
        def data_received(self, data):
            ...
            for gamePacket in d.nextPackets():
                if isinstance(gamePacket,gamePackage.GameCommandPacket):
                    print(gamePacket.command_line)
                    command = gamePacket.command_line;
                    print("command")
                    self.game.command(command)
                elif isinstance(gamePacket,gamePackage.GameInitPacket):
                    print("1")
