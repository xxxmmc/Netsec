...
class EchoServer(asyncio.Protocol):
    ...
    def connection_made(self, transport):
        ...
        self.game = EscapeRoomGame(output=self.output)
        self.game.create_game()
        self.game.start()
        self.loop = asyncio.get_event_loop()
        self.loop.create_task(self.startflying())

    def data_received(self, data_bytes):
        #deserializer = PacketType.Deserializer()
        #deserializer.update(data_bytes)
        #data = list(deserializer.nextPackets())
        #mycommand = data[0].comm
        #output = self.game.command(mycommand)
    ...
    def output(self, mystr):
        print(mystr)
        respkt = my_packet_file.GameResponsePacket.create_game_response_packet(str, self.game.status)
        self.transport.write(respkt.__serialize__())

    def set_game(self, game):
        self.game = game

    async def startflying(self):
        await asyncio.wait([asyncio.ensure_future(a) for a in self.game.agents])

async def main(args):
    loop = asyncio.get_event_loop()
    await playground.create_server(EchoServer, 'localhost', 8855)
