...
class POOP(StackingProtocol):
    def __init__(self, mode):
        ...
        self.recv_queue = []
        self.recv_wind_size = 2
        self.recv_next = None
        self.send_buff = []
        self.send_queue = []
        self.send_wind_size = 2
    ...
    def connection_made(self, transport):
        ...
        if self._mode == "client":  # client send first packet
            ...
            print('Client send SYN:'+str(self.SYN))

            # TODO:start timer
            self.loop.create_task(self.handshake_timeout_check())
            ...
            if pkt.status == 0:
                if pkt.SYN:  # server LISTEN and handshake get the packet from the client
                    ...
                    handshake_pkt.hash = binascii.crc32(
                        handshake_pkt.__serialize__()) & 0xffffffff
                    self.transport.write(handshake_pkt.__serialize__())
                    print('Server send SYN/ACK SYN: '+str(self.SYN)+' ACK: '+str(pkt.SYN + 1))
                    self.status = "SYN_SENT"
                    self.send_buff.append(handshake_pkt.__serialize__())  # @
                else:
    ...
    async def handshake_timeout_check(self):
        ...
        while count < 3:
            ...
            if self.status == "ESTABLISHED" or self.status == "FIN_SENT" or self.status == "DYING":
                print("Do not need resend handshake")
                return
            print("!!!!!!!!!!!!")
            ...
            print("resend!!!!!!!!!!")
            self.transport.write(handshake_pkt.__serialize__())
            print("resend packet:SYN"+str(self.SYN))
    ...
    def data_pkt_recv(self, pkt):
        ...
        if pkt.ACK:
            ...
            print("#######Get ACK packet,IN: ACK=" + str(pkt.ACK))
            if self.send_queue:
        ...
        if pkt.seq <= self.recv_next + self.recv_wind_size:
            ...
            print("Out bound packet!!!!!")
            return

        print("INPUT: SEQ=" + str(pkt.seq))
        print("Current recvwindow:"+str(self.recv_next))
        ...
        if pkt.seq < self.recv_next:
            ...
            print("Already received, Stop resend! OUTPUT: ACK=" + str(pkt.seq))
            return
        ...
        while self.recv_queue:
            if self.recv_queue[0].seq == self.recv_next:
                ...
                ack_pkt = DataPacket(ACK=self.recv_next, hash=0)
                ack_pkt.hash = binascii.crc32(ack_pkt.__serialize__()) & 0xffffffff
                self.transport.write(ack_pkt.__serialize__())
                print("OUTPUT: ACK=" + str(self.recv_next))
                while self.recv_queue:
                    if self.recv_queue[0].seq == self.recv_next:
